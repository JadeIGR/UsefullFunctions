...
# IDEA OF MAKING A MORE GENERAL FUNCTION TO CHECK THE TYPE OF THE DATAs (put the type - condition -in parameter)
# type_is_good<- function (db,nomColNum, condition, msg_erreur){
#   msg <-""
#   for (i in 1:nrow(db) ){
#     if(condition & !is.na(db[i,nomColNum]) & db[i,nomColNum] != "" & db[i,nomColNum] != "NA" ){
#       msg <- paste("Erreur ! Ligne :" , i+1 , "& Valeur:",db[i,nomColNum] )
#       print(msg)
#     }
#   }
#   if (msg == ""){
#     print("tout est ok pour cette colonne")
#   } else{print(msg_erreur)}
# }
# 
# type_is_good(baseDDR,'OS','!is.numeric(db[i,nomColNum]')


## CODE FOR CHECKING IF ALL THE CATEGORIES ARE WELL WRITTEN

#PARTIE 1: entrer le nom des catégories d'une variable (exemple: F et M pour 'sex')
...
#PARTIE 2: vérifier que toutes les données inscrites appartiennent bien à ce vecteur de catégorie. Si non, alors cela signifie qu'il y a eu une erreur de saisie.
